...
export async function GapReport() {
  ...
  try {
    ...
    // Note: Using minimal fields that should exist in both old and new schema
    ...
    const seriesWithIssues = await Promise.all(
      allSeries.map(async (s) => {
        try {
          ...
          // If issues query fails, return series with empty issues array
          console.warn(`Failed to fetch issues for series ${s.id}:`, error);
          return { ...s, issues: [] };
        }
      })
    );
  ...
  return (
    ...
    // Gracefully handle schema mismatch or database errors
    console.error('GapReport error:', error);
    return null;
  }
}
